...
import com.yongjun.stock.annotation.DuplicateSubmissionToken;
...
public class TokenGenerator {
     ...
     * 保存token值的默认命名空间
     */
    public static final String TOKEN_NAMESPACE = "shiro:tokens:";
    ...
    public static boolean validToken(HttpServletRequest request) {
        CContext cContext = CContext.getCurrentContext();
        String tokenName = cContext.getParameter(TOKEN_KEY_FIELD);
        String token = cContext.getParameter(TOKEN_VALUE_FIELD);
        ...
        if (!token.equals(cacheToken)) {
            logger.error("yongjun.internal.invalid.token Form token {} does not match the session token {}.", token, cacheToken);
            return false;
        }
